...
<div class="container">
	...
	<c:choose>
		<c:when test="${not empty user_id}">
			...
			<!-- 시큐리티 설정에
			logout-url="/security2/logout" 와 같으므로 로그아웃의 
			URL은 아래와 같이 설정한다. 
			 -->
		...
		경우 동일한 값으로 action속성을 설정해야 한다.
		설정하지 않았다면 default값이 "/login"이므로 이와 같이 기술한다.
		
		만약 해당 설정 값을 추가했다면 컨텍스트 루트 경로를 제외한 나머지
		경로를 통해 action속성을 기술해야 한다. 
		 -->
			...
			<form:form name="loginFrm" action="../myloginAction" method="post">
				...
				password-parameter="pass" 와 같이 설정돼있으므로 name속성은 아래와 같이 기술한다.
				 -->
				<p>
